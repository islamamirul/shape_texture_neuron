...
def val_imgen(model, args):
    ...
    state_dict = torch.load(args['current_snapshot'], map_location={'cuda:0': 'cuda:1'})
    ...
    with torch.no_grad():
        for inputs, gt, image_id in tqdm(
            ...
            for lp, lt in zip(output, gt):
                hist += fast_hist(lt.flatten(), lp.flatten(), 2)
        score, class_iou = scores(hist, n_class=2)
...
if __name__ == '__main__':
    ...
    # args['current_snap_name'] = 'resnet50_imagenet_pretrained_classification_voc12_shape_lr002_1layersreadout_sd_ckpt_19830'
