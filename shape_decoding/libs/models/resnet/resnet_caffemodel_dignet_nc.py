...
class ResNet32sCaffeModelDIGNet(nn.Module):
	...
	#PropagatorGate(inch_lr,outch_lr, inch_sr,outch_sr, outch_fbout,apk=5,convk=3)
        if self.sb<=1: 
            self.add_module('layer0_fbl',PropagatorGate(_num_classes,_num_classes,64,_num_classes,_num_classes))
        if self.sb<=2: 
            self.add_module('layer1_fbl',PropagatorGate(_num_classes,_num_classes,256,_num_classes,_num_classes))
        if self.sb<=3: 
            self.add_module('layer2_fbl',PropagatorGate(_num_classes,_num_classes,512,_num_classes,_num_classes))
        if self.sb<=4: 
            self.add_module('layer3_fbl',PropagatorGate(_num_classes,_num_classes,1024,_num_classes,_num_classes))
        if self.sb<=5: 
            self.add_module('layer4_fbl',PropagatorGate(_num_classes,_num_classes,2048,_num_classes,_num_classes))
  
        #ModulatorGate(fb_channels,xi_channels,apk=5,convk=3)
        if self.sb<=1: 
            self.add_module('layer0_fbm',_modulator(_num_classes,3,apk=_modulator_apk))
        if self.sb<=2: 
            self.add_module('layer1_fbm',_modulator(_num_classes,64,apk=_modulator_apk))
        if self.sb<=3: 
            self.add_module('layer2_fbm',_modulator(_num_classes,256,apk=_modulator_apk))
        if self.sb<=4: 
            self.add_module('layer3_fbm',_modulator(_num_classes,512,apk=_modulator_apk))
        if self.sb<=5: 
            self.add_module('layer4_fbm',_modulator(_num_classes,1024,apk=_modulator_apk))
        if self.sb<=6: 
