...
class ResNet32sCaffeModelDIGNet(nn.Module):
	...
	#PropagatorGate(inch_lr,outch_lr, inch_sr,outch_sr, outch_fbout,apk=5,convk=3)
        if self.sb<=1: 
            self.add_module('layer0_fbl',PropagatorGate(64, 32, 64, 16, 32))
        if self.sb<=2: 
            self.add_module('layer1_fbl',PropagatorGate(128, 64, 256, 32, 64))
        if self.sb<=3: 
            self.add_module('layer2_fbl',PropagatorGate(256, 128, 512, 64, 128))
        if self.sb<=4: 
            self.add_module('layer3_fbl',PropagatorGate(256, 256, 1024, 128, 256))
        if self.sb<=5: 
        ...
        if self.sb<=1: 
            self.add_module('layer0_fbm',_modulator(32,3,apk=_modulator_apk))
        if self.sb<=2: 
            self.add_module('layer1_fbm',_modulator(64,64,apk=_modulator_apk))
        if self.sb<=3: 
            self.add_module('layer2_fbm',_modulator(128,256,apk=_modulator_apk))
        if self.sb<=4: 
            self.add_module('layer3_fbm',_modulator(256,512,apk=_modulator_apk))
        if self.sb<=5: 
