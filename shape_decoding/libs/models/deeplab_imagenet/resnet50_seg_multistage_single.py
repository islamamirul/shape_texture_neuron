...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        '''self.stage1_feature = nn.Conv2d(256, 64, 1, bias=False)
        self.stage2_feature = nn.Conv2d(512, 64, 1, bias=False)
        self.stage3_feature = nn.Conv2d(1024, 64, 1, bias=False)'''
        # self.stage1_feature = nn.Conv2d(2048, 20, 1, bias=False)

        self.classifier = PosENet(input_dim=64)
    ...
    def forward(self, x):
        ...
        x = self.stage1(x)
        #stage1_f = self.stage1_feature(x)

        # x = self.stage2(x).detach()
        #stage2_f = self.stage2_feature(x)

        # x = self.stage3(x)
        #stage3_f = self.stage3_feature(x)
